...
if (isset($qode_options['enable_popup_menu'])){
...
if(isset($qode_options['paspartu_header_alignment']) && $qode_options['paspartu_header_alignment'] == 'yes' && isset($qode_options['paspartu']) && $qode_options['paspartu'] == 'yes'){$paspartu_header_alignment = true;}

$header_in_grid = true;
if(isset($qode_options['header_in_grid'])){
    if ($qode_options['header_in_grid'] == "no"){ $header_in_grid = false; }
    if ($paspartu_header_alignment) { $header_in_grid = false; }
}

$menu_item_icon_position = "";
if(isset($qode_options['menu_item_icon_position'])){$menu_item_icon_position = $qode_options['menu_item_icon_position']; }

$menu_position = "";
if(isset($qode_options['menu_position'])){$menu_position = $qode_options['menu_position']; }

$centered_logo = false;
if (isset($qode_options['center_logo_image'])){ if($qode_options['center_logo_image'] == "yes" && $qode_options['header_bottom_appearance'] !== "stick_with_left_right_menu") { $centered_logo = true; }};
...
if(isset($qode_options['header_bottom_appearance']) && ($qode_options['header_bottom_appearance'] == "stick_with_left_right_menu")){
		$logo_wrapper_style = 'style="width:'.(esc_attr($qode_options['logo_width'])/2).'px;"';
    $divided_left_menu_padding = 'style="padding-right:'.(esc_attr($qode_options['logo_width'])/4).'px;"';
    $divided_right_menu_padding = 'style="padding-left:'.(esc_attr($qode_options['logo_width'])/4).'px;"';
}
if($qode_options['center_logo_image'] == "yes" && $qode_options['header_bottom_appearance'] == "regular"){
	$logo_wrapper_style = 'style="height:'.(esc_attr($qode_options['logo_height'])/2).'px;"';
}
...
if(isset($qode_options['header_background_transparency_sticky']) && $qode_options['header_background_transparency_sticky'] != ""){
	$header_color_transparency_on_scroll = esc_attr($qode_options['header_background_transparency_sticky']);
}

$header_separator = qode_hex2rgb("#c0c0c0");
if(isset($qode_options['header_separator_color']) && $qode_options['header_separator_color'] != ""){
	$header_separator = qode_hex2rgb(esc_attr($qode_options['header_separator_color']));
}
...
if(isset($qode_options['header_botom_border_in_grid']) && $qode_options['header_botom_border_in_grid'] == "yes" && get_post_meta($id, "qode_header_bottom_border_color", true) != ""){
	$header_bottom_border_style = 'style="border-bottom: 1px solid '.esc_attr(get_post_meta($id, "qode_header_bottom_border_color", true)).';"';
}
...
<?php if($enable_vertical_menu) { ?>
	...
	if(get_post_meta($id, "qode_page_vertical_area_background", true) != ""){
		$page_vertical_area_background = 'style="background-color:'.esc_attr(get_post_meta($id, "qode_page_vertical_area_background", true)).';"';
	}
    
	?>
	<aside class="vertical_menu_area<?php echo esc_attr($vertical_area_scroll); ?> <?php echo esc_attr($header_style); ?>" <?php echo $page_vertical_area_background; /* dynamically generated style attr. Note that is escaped on line 290 */ ?>>
		<div class="vertical_menu_area_inner">
			<?php if(isset($qode_options['vertical_area_type']) && $qode_options['vertical_area_type'] == 'hidden') { ?>
            ...
            <?php if($vertical_area_background_image != ""){ ?>
			    <div class="vertical_area_background preload_background" <?php echo 'style="background-image:url('.esc_url($vertical_area_background_image).');"'; ?>></div>
            <?php } ?>
			...
			<?php if (!(isset($qode_options['show_logo']) && $qode_options['show_logo'] == "no")){ ?>
				<div class="vertical_logo_wrapper">
					...
					?>
					<div class="q_logo_vertical" style="height: <?php echo intval($qode_options['logo_height'])/2 ?>px;">
					 ...
					 <?php } ?>
						<div class="form_holder_outer">
							<div class="form_holder">
								...
								<!--<input type="submit" class="qode_search_submit" value="<?php /*if (isset($qode_options['header_icon_pack'])) { $qodeIconCollections->getSearchIconValue($qode_options['header_icon_pack']); } */?>"/>-->
...
<!--insert end-->
	<div class="header_top_bottom_holder">
		<?php if($display_header_top == "yes"){ ?>
			<div class="header_top clearfix" <?php echo $header_top_color_per_page; /* dynamically generated style attr. Note that is escaped on line 211-218 */ ?> >
				<?php if($header_in_grid){ ?>
				<div class="container">
					<div class="container_inner clearfix" >
						<?php } ?>
		...
		<?php } ?>
		<div class="header_bottom clearfix <?php if($menu_item_icon_position=="top"){echo 'with_large_icons ';} if($menu_position == "left" && $header_bottom_appearance != "fixed_hiding" && $header_bottom_appearance != "stick menu_bottom" && $header_bottom_appearance != "stick_with_left_right_menu"){ echo 'left_menu_position';} ?>" <?php echo $header_color_per_page; /* dynamically generated style attr. Note that is escaped on line 211-218 */ ?> >
			<?php if($header_in_grid){ ?>
			<div class="container">
				<div class="container_inner clearfix" <?php echo $header_bottom_border_style; /* dynamically generated style attr. Note that is escaped on line 236 */ ?>>
					<?php } ?>
                    <?php if($header_bottom_appearance == "stick_with_left_right_menu") { ?>
                        <nav class="main_menu drop_down left_side" <?php echo $divided_left_menu_padding; /* dynamically generated style attr. Note that is escaped on line 147 */ ?>>
                            <?php
					...
					<div class="header_inner_left">
						...
						<?php if (!(isset($qode_options['show_logo']) && $qode_options['show_logo'] == "no")){ ?>
							<div class="logo_wrapper" <?php echo $logo_wrapper_style; /* dynamically generated style attr. Note that is escaped on line 146 */ ?>>
								<?php
								if (isset($qode_options['logo_image']) && $qode_options['logo_image'] != ""){ $logo_image = $qode_options['logo_image'];}else{ $logo_image =  get_template_directory_uri().'/img/logo.png'; };
								if (isset($qode_options['logo_image_light']) && $qode_options['logo_image_light'] != ""){ $logo_image_light = $qode_options['logo_image_light'];}else{ $logo_image_light =  get_template_directory_uri().'/img/logo.png'; };
								if (isset($qode_options['logo_image_dark']) && $qode_options['logo_image_dark'] != ""){ $logo_image_dark = $qode_options['logo_image_dark'];}else{ $logo_image_dark =  get_template_directory_uri().'/img/logo_black.png'; };
					...
					<?php if($header_bottom_appearance == "stick_with_left_right_menu") { ?>
                        <nav class="main_menu drop_down right_side" <?php echo $divided_right_menu_padding; /* dynamically generated style attr. Note that is escaped on line 148 */ ?>>
                            <?php
...
<!-- insert start-->
                                    <?php if(isset($qode_options['enable_search']) && $qode_options['enable_search'] == 'yes') {
                                        ...
                                        <a class="<?php echo esc_attr($search_type_class); ?>" href="javascript:void(0)">
											<?php if(isset($qode_options['header_icon_pack'])){ $qodeIconCollections->getSearchIcon($qode_options['header_icon_pack']); } ?>
...
<!--insert end-->
										<?php if($enable_popup_menu == "yes"){ ?>
											<a href="javascript:void(0)" class="popup_menu <?php echo esc_attr($header_button_size).' '.esc_attr($popup_menu_animation_style); ?>"><span class="popup_menu_inner"><i class="line">&nbsp;</i></span></a>
										<?php } ?>
...
<?php } else{?>
	<header class="page_header <?php if($display_header_top == "yes"){ echo 'has_top'; }  if($header_top_area_scroll == "yes"){ echo ' scroll_top'; }?> <?php if($centered_logo){ echo " centered_logo"; } ?> <?php echo esc_attr($header_bottom_appearance); ?>  <?php echo esc_attr($header_style); ?> <?php if(is_active_sidebar('header_fixed_right')) { echo 'has_header_fixed_right'; } ?>">
		...
		<div class="header_inner clearfix">
			...
			<div class="header_bottom clearfix" <?php echo $header_color_per_page; /* dynamically generated style attr. Note that is escaped on line 211-218 */ ?> >
				<?php if($header_in_grid){ ?>
				<div class="container">
					<div class="container_inner clearfix" <?php echo $header_bottom_border_style; /* dynamically generated style attr. Note that is escaped on line 236 */ ?>>
						<?php } ?>
...
<?php if($qode_options['show_back_button'] == "yes") {
	...
	<a id='back_to_top' class="<?php echo esc_attr($q_back_to_top_button_styles);?>" href='#'>
			<span class="qode_icon_stack">
				<?php if(isset($qode_options['show_back_button_icon_set'])) {
					$qodeIconCollections->getBackToTopIcon($qode_options['show_back_button_icon_set']);
				} ?>
			</span>
	</a>
<?php } ?>
...
if(isset($qode_options['paspartu_on_top']) && $qode_options['paspartu_on_top'] == 'no' && isset($qode_options['paspartu_on_bottom_slider']) && $qode_options['paspartu_on_bottom_slider'] == 'yes'){
